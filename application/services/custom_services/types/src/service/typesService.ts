...
public  GpGetNounById(req: Request, callback){
    new CustomLogger().showLogger('info', 'Enter into typesService.ts: GpGetNounById')
     let  typesId = req.params.id;
     types.GpGetNounById(typesId,(response)=>{
             new CustomLogger().showLogger('info', 'Exit from typesService.ts: GpGetNounById')
         callback(response);
         });
    }
