...
public async GpGetNounById(typesId, callback){
    
    new CustomLogger().showLogger('info', 'Enter into typesDao.ts: GpGetNounById');
    ...
    this.types.findById(typesId).then((result)	=>	{
        ...
        new CustomLogger().showLogger('info', 'Exit from typesDao.ts: GpGetNounById');
